...
- [ ] `home_hero_cta_surgery` — "Surgical Costs" CTA on home page
- [ ] `home_people_heading` — "Notable people with Endometriosis" heading
- [ ] `home_urgent_btn` — Help/warning button in topbar
- [ ] `stats_context` — Mortality stat tooltip disclaimer

### Hardcoded English text needing `data-i18n` attributes
- [ ] `base.html`: Settings button — `aria-label="Settings"` and `title="Settings"` need i18n keys
- [ ] `base.html`: "Language" label in settings panel — no `data-i18n`
- [ ] `base.html`: "Theme" label in settings panel — no `data-i18n`
- [ ] `base.html`: "Light / Dark" theme toggle label — no `data-i18n`
- [ ] `home.html`: "Here's where to start." subheading — not wrapped in i18n
- [ ] `home.html`: "See all" link (notable-people section) — no `data-i18n`
- [ ] `home.html`: "Graphics via" carousel credit — hardcoded

### Translation backlog (84 keys pending across 26 languages)
...
- [ ] RTL carousel arrows — flip chevrons for Arabic (`[dir="rtl"] .carousel-arrow svg { transform: scaleX(-1); }`)
...
- [ ] Consider FAQ schema markup (JSON-LD) for FAQ page
...
- [ ] Print stylesheet for content pages
- [ ] RSS/Atom feed generation in build.py
- [ ] Sitemap `<lastmod>` dates from frontmatter
...
- [ ] Add structured data for Organization (schema.org)
...
- [ ] Consider CDN cache headers in Cloudflare Pages `_headers` file
